...
{ 
    public function storeRequest(Request $request)
    {
        ...
        $expiresAt = Carbon::now()->addMinutes($request->deadline_minutes);
        ...
        return redirect()->back()->with('success','Document request sent.');
    }
    ...
    {
        $user = Auth::user();
         if ($user->id !== $documentRequest->target_user_id) abort(403);
    ...
    {
        $user = Auth::user();
        if ($user->id !== $documentRequest->requester_id && $user->role->name !== 'super_admin') abort(403);
