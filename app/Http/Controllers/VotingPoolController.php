    ...
    public function data()
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('action', function ($row) {
                ...
                $tenant = Tenant::where('child_id', auth()->user()->id)->first();
                $btns = '';
                $btns .= '<a href="' . route('' . auth()->user()->custom_role . '.pools.show', $row->id) . '" class="btn btn-sm btn-primary">View</a>';

                if ($tenant == null) {
                    $btns .= '<a href="' . route('familyOwner.pools.edit', $row->id) . '" class="btn btn-sm btn-warning">Edit</a>';
                    $btns .= '<form method="POST" action="' . route('familyOwner.pools.destroy', $row->id) . '" style="display:inline;">
